...
13. **The haze comes back; the water it stands for does not.** Re-condensation restores `fog` fast, and it must, or drawings never fade. But fog is worth liquid, so at that rate it is an unlimited water supply: a drop sweeps its track, sheds, the wet track re-fogs in seconds, the next drop sweeps it again. Measured after one drag and nothing else: thirteen drops ran for ninety seconds while the liquid on the glass *rose* from 141 to 526. So the water the fog holds is its own field, `humid`, topped up by Steam and otherwise trickling back at a small fraction of the haze's rate; the finger and the flows harvest that, never `fog`. See PHYSICS.md §5c.
14. **The head cap is a performance budget, never a metering valve.** If a wiped ridge cannot bead all at once it beads in instalments instead, and keeps making drops for as long as the water lasts — which is exactly what "the glass keeps producing water" looks like. See PHYSICS.md §7a.
15. **Water a flow sheds stays part of that flow.** Excess above the size glass can hold, and the last of a head that dwindles away, are laid down as *track*, claimed in the flow map; and water sagging out of a track carries the track with it. Dropped as loose water on fresh glass it simply beads again a moment later, one line at a time.
16. **Trail width must sit below the drop's own radius, not above it.** The anti-staircase floor was wider than any drop this produces, so every streak came out the same width and three times fatter than its head. Lower the floor, and carry the visible difference in how wet the track is left — which scales with the head's mass. See PHYSICS.md §12b.
