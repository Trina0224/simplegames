...
last group holds the two branches, and the slider walks them — 30 members each,
sampled geometrically by perilune from one continuation that runs all the way to
the lunar surface. The NRHO preset is a member of it, not a separate discovery.

`src/family3d.js` is generated by `tools/family.mjs` and every number in it is
measured from the propagator afterwards. Re-flown by the suite, all 60 members
close (worst 5.3e-9 at the deep end, where the orbits are violently unstable),
and none passes inside the Moon — the deepest clears it by metres.
...
```text
L1   perilune 51 120 → 1 737 km    |z| 2 049 → 95 925 km            slenderness 0.17 → 4.56
L2   perilune 50 922 → 1 737 km    |z| 2 323 → 77 778 → 66 561 km              0.10 → 7.40
```

Both branches used to stop well short of that — L2 at 7412 km — until the
continuation was rewritten to hold nothing at all. See *A fold is the end of a
parameterisation, not of a family*.
...
The L2 halo branch stopped at a perilune of 7412 km, and for a while that looked
like the family ending. It was not.

`haloBranch` walks the family by holding one component of the initial state and
correcting the other two, switching the held component at the fold where the
family stops being a function of z0. The L2 family turns over **twice**, and the
second fold is in x0 — the component the branch had switched to. Measured, its
last two members are 1.0e-3 apart in z0 and 2.5e-5 apart in x0: a family running
very nearly perpendicular to the parameter it is being asked to advance. The
stride collapsed and the walk ended. Halving it does not help; at a minimum
stride of 1e-7 it reaches 6729 km and stops for the same reason.

`haloArc` holds nothing. The unknown is the whole initial state (x0, z0, vy0);
the two residuals are (vx, vz) at the next y = 0 crossing, exactly as before; and
the third equation is the arclength condition
...
family's own tangent. The tangent is the null vector of the 2×3 Jacobian — for a
2×3 matrix, the cross product of its rows — with its sign fixed by continuity.
Nothing in that construction cares which way the family is leaning, so a fold in
any component stops being an event at all.

Both branches now run from the Richardson seed to the lunar surface. L2 clears it
by **1 metre** at a period of 5.920 days; L1 by 4 metres. The check that matters
is that the answer does not depend on the stride: two walks four times apart in
step size agree on that terminus to 1 m and 1e-4 days. What is being followed is
the family, not the discretisation.

The sign of the first tangent had to be taken from the branch already walked. The
null vector's sign is arbitrary, and the first version cheerfully walked back *up*
the family — 4000 members of it — reporting perilunes that grew instead of
shrinking. It was obvious only because the numbers were printed.

Two consequences worth recording:

**The family slider was sampling by index.** The two continuations do not step at
the same rate — L2 is 64 members by held component and 3023 by arclength — so an
evenly-indexed sample of 34 put 33 of them below 7254 km and jumped straight there
from 50 922 km. It samples geometrically by perilune now, which is both the
monotonic parameter of the branch and the thing a reader watches change.

**`periluneKm` was stored rounded to whole kilometres.** At the deep end that is
not a rounding, it is a sign error: the deepest member stored 1737 against a lunar
radius of 1737.4 and read as being *inside* the Moon. Stored to metres now, and
the readout and the caution panel switch to metres below a kilometre — "0 km over
the Moon" is not a small altitude, it is no altitude at all.

### Artemis: mission context on an orbit that was found, not drawn

`ARTEMIS_DEMO_SPEC.md` returns to one rule over and over: *"Do not tune a curve
visually to NASA's published closest/farthest distances."* So the Gateway demo
does exactly two things — it runs the same continuation the family slider browses,
and it picks the member whose period is nearest the 6.5 days NASA publishes.

What that member turns out to be:

| | simulated | NASA reference |
|---|---|---|
| period | 6.5001 d | ~6.5 d |
| near pass | 1345 km | ~1500 km |
| far pass | 70 858 km | ~70 000 km |

The proof that this is selection rather than fitting is that NASA's three
published figures **agree with each other about which member to pick**. Choosing
by period lands on 6.5001 d; choosing by the near-pass figure would have landed on
6.5582 d, and by the far-pass figure on 6.3568 d — all within a fifth of a day of
each other on a family that spans 5.9 to 14.8 days. A curve fitted to one number
would not do that. A family that goes where Gateway's goes does.

The published figures and the measured ones are never in the same sentence, never
in the same typeface, and the measured line always says where it came from — the
spec's acceptance test 4. The CAPSTONE demo is deliberately the **same orbit**:
CAPSTONE flew a lunar NRHO as a pathfinder for the family Gateway is planned for,
so its published geometry and Gateway's are the same geometry, and pretending
otherwise by showing a second, slightly different curve would be the fabrication
the spec is guarding against.

One thing the old `NRHO3D` comment claimed had to be retracted. It said
reproducing Gateway's geometry *"would need an ephemeris model this project does
not have."* That was wrong, and only the deeper continuation showed it: the ideal
CR3BP family passes straight through 6.5 days and a 1500 km near pass. What still
needs an ephemeris is the **operational** orbit — the 9:2 resonance is a resonance
with the real Sun–Earth–Moon system, and station keeping is what holds it.

**The low lunar orbit is flown by the same integrator.** The spec permits a
Moon-centred two-body illustration if it is labelled as one; it is better not to
need the label. The circular initial state is worked out analytically and then
handed to the same CR3BP propagator as everything else, so its altitude wanders by
a measured 0.13 km over one Gateway orbit rather than staying exactly circular —
79.5 revolutions to the NRHO's one, which is the comparison the demo exists to
make.

The first version of that initial state fell into the Moon inside a tenth of a
revolution, from every altitude tried. The rotating-frame conversion is
`v_rot = v_inertial − ω × r`, and the Moon's *own* orbital velocity appears in both
terms and cancels:

```text
inertial   (0, 1−μ,             0) + (0, √(μ/r), 0)
frame      (0, 1−μ + r,         0)
rotating   (0, √(μ/r) − r,      0)
```

Keeping the `1−μ` in the frame term and losing it from the inertial one is a
spacecraft launched at 62% of circular speed. It is a one-symbol slip that reads
...
The deepest member of the L1 family passes **4 metres above the lunar surface** —
the family really does terminate in a collision orbit, and the continuation is
stopped at the ground rather than at a tidy round altitude. It is kept: the family
is not trimmed for presentation, because a truncated family is a claim about the
physics that the physics did not make. (It used to be 51 km, which was not a
choice either — it was where the held-component continuation happened to stop.)
...
> Closest approach 4 m above the Moon. This is an idealized CR3BP result: the
...
(It is now an interior member: pseudo-arclength continuation carries the same
family another 3023 members past it, to the lunar surface.)
...
**It is not Gateway's orbit**, but the reason I first gave was wrong. I wrote that
reproducing Gateway's geometry "needs an ephemeris model this project does not
have"; continuing the same family past the fold it was stopping at reaches 6.5001
days and a 1345 km near pass without leaving the ideal CR3BP. What still needs an
ephemeris is the *operational* orbit — the 9:2 resonance is a resonance with the
real Sun–Earth–Moon system, and station keeping is what holds it. The spec says
not to claim operational fidelity, so none is claimed.
...
| Gateway-like NRHO | `[0.9874065435015671, 0, 0.008005872776257781, 0, 1.713090117697641, 0]`, T = 1.4968470055984293 TU |
| | C = 3.047589116, residual 1.2e-13, closes 6.1e-11, drift 1.1e-13, 906 steps / 1 rejected |
| the L2 branch, held component | 64 members from a Richardson seed; stalls at a fold in x0, perilune 7 412 km |
| the same branch, pseudo-arclength | 3 023 members further, to the lunar surface: 1 737.401 km, T = 5.920 d |
| that terminus against the stride | ds 4e-3 and ds 1e-3 agree to 1 m and 1e-4 d |
| the browsable families | 60 members; all close when re-flown, worst 5.3e-9; none inside the Moon |
| the NRHO preset | an interior member of the L2 family, perilune 7 412 km |
| the closure gate | caught members with residual 1e-12 whose orbits missed themselves by 2.4 DU |
| the deepest members kept | L1 clears the lunar surface by 4 m, L2 by 2 m — kept, and flagged as idealized |
